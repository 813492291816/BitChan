...
logger = logging.getLogger("bitchan.message_summary")
            ...
            if message:
                ...
                for i, each_file in enumerate(file_order, start=1):
                    ...
                    if each_file in media_info:
                        ...
                        attachment_info[each_file] = media_info[each_file]
...
def generate_header_file_info(message):
    ret_str = ""
...
def get_reply_link_html(
    ...
    try:
        ...
        ret_str += '<a class="crosslink reply-tooltip{cls}" style="{sty};{esty}" href="/thread/{ch}/{th}#{pid}">{lstr}' \
                   '<div class="reply-main">'.format(
            cls=classes_str,
            sty=style_str,
            esty=extra_style,
            ch=message.thread.chan.address,
            th=message.thread.thread_hash_short,
            pid=message.post_id,
            lstr=link_str)
        ...
        ret_str += '<div class="reply-header link">{}</div>' \
                   '<div class="reply-break"></div>'.format(
                    popup_message_header(message, external_thread=external_thread))

        ret_str += generate_header_file_info(message)
        ...
        if message.popup_html and not message.regenerate_popup_html:
            ...
            with session_scope(DB_PATH) as new_session:
                ...
                message_edit.popup_html = generate_reply_link_html(message)
...
def generate_reply_link_html(message):
    file_order, attach, number_files = attachment_info(message.message_id)
    ret_str = ''
    ...
    ret_str += '<div class="reply-text themed">{}</div></div></a>'.format(
        popup_message_content(message))

    return ret_str


def popup_message_header(message, external_thread=False):
    ...
    try:
        ...
        logger.exception("Error in popup_message_header(): {}".format(err))
        str_return = "Error: Could not parse post header"

    return str_return


def popup_message_content(message):
    ...
    try:
        ...
        if message.message:
            ...
            soup = BeautifulSoup(str_return, features="html5lib")
        ...
        logger.exception("Error in popup_message_content(): {}".format(err))
